...
class DeepseekV32Attention(MLAAttention):
    ...
    def __init__(
        ...
        #   * not supported (FlashMLA sparse, SM90/Hopper): fp8_ds_mla cache
        ...
        assert is_quantized_kv_cache(self.kv_cache_dtype), (
            ...
            "MLA backend. Launch with --kv-cache-dtype fp8 (SM100) or "
            "--kv-cache-dtype fp8_ds_mla (Hopper)."
        )
        ...
        if not self._fp8_query:
            assert self.kv_cache_dtype == "fp8_ds_mla", (
                ...
                "(FlashMLA, e.g. Hopper) requires the fp8_ds_mla KV cache "
                "layout. Launch with --kv-cache-dtype fp8_ds_mla."
            )
